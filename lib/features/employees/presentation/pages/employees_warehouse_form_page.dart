...
class EmployeesWarehouseFormPage extends StatefulWidget {
  final EmployeeWarehouse? existing;
  const EmployeesWarehouseFormPage({super.key, this.existing});

  @override
  State<EmployeesWarehouseFormPage> createState() => _EmployeesWarehouseFormPageState();
}

class _EmployeesWarehouseFormPageState extends State<EmployeesWarehouseFormPage> {
  ...
  void initState() {
    ...
    if (widget.existing != null) {
      ...
      _positionController.text = widget.existing!.position ?? '';
      _departmentController.text = widget.existing!.department ?? '';
  ...
  Future<void> _save() async {
    ...
    try {
      final repo = sl<EmployeeWarehouseRepository>();

      final ci = _ciController.text.trim();
      if (ci.isNotEmpty) {
        final exists = await repo.existsCi(ci, excludeId: widget.existing?.id);
        if (exists) {
          throw Exception('Ya existe un empleado de almacén con este CI');
        }
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(isEdit ? 'Editar Empleado de Almacén' : 'Nuevo Empleado de Almacén')),
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                  labelText: 'Cargo (Ej: Almacenista, Jefe de Almacén)',
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                  labelText: 'Departamento (Ej: Recepción, Despacho)',
              ...
              ElevatedButton.icon(
                ...
                label: Text(_saving ? 'Guardando...' : (isEdit ? 'Actualizar' : 'Crear Empleado')),
              ),
            ],
          ),
        ),
