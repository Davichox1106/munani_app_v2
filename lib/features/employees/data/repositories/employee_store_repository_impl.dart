...
import '../datasources/employee_store_local_datasource.dart';
import '../datasources/employee_store_remote_datasource.dart';
import '../models/employee_store_local_model.dart';

/// Implementación del repositorio de Employee Store con patrón Offline-First
class EmployeeStoreRepositoryImpl implements EmployeeStoreRepository {
  final EmployeeStoreLocalDataSource localDataSource;
  final EmployeeStoreRemoteDatasource remoteDatasource;
  ...
  @override
  Stream<List<EmployeeStore>> getAll() async* {
    try {
  ...
  @override
  Stream<List<EmployeeStore>> getAllIncludingInactive() async* {
    try {
      if (await networkInfo.isConnected) {
        _syncInBackground();
      }
  ...
  @override
  Future<List<EmployeeStore>> search(String query) async {
    try {
  ...
  @override
  Future<EmployeeStore?> getByEmail(String email) async {
    try {
      ...
      if (await networkInfo.isConnected) {
        ...
        if (remote != null) {
          final localModel = EmployeeStoreLocalModel.fromEntity(remote);
  ...
  Future<EmployeeStore> create({
    ...
    try {
      ...
      final newEmployee = EmployeeStore(
        id: uuid,
        name: name,
      ...
      final localModel = EmployeeStoreLocalModel.fromEntity(newEmployee)
        ..isSynced = false;
      await localDataSource.saveEmployee(localModel);
  ...
  @override
  Future<EmployeeStore> update(EmployeeStore employee, String updatedBy) async {
    try {
      ...
      final localModel = EmployeeStoreLocalModel.fromEntity(updatedEmployee)
        ..isSynced = false;
      await localDataSource.saveEmployee(localModel);
  ...
  Future<void> syncEmployees() async {
    ...
    try {
      AppLogger.info('📋 Sincronizando empleados bidireccional (Store)...');
      ...
      for (final remoteEmployee in remoteEmployees) {
        try {
          final localModel = EmployeeStoreLocalModel.fromEntity(remoteEmployee);
          localModel.markAsSynced();
          await localDataSource.saveEmployee(localModel);
        } catch (e) {
      ...
      AppLogger.info('✅ Sincronización bidireccional completada (Store)');
    } catch (e) {
